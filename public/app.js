...
async function generateNewSolution() {
    try {
        ...
        // data access outside games.json
        const platformID = gameData.games[randNum].platform;
        const genreIDArray = gameData.games[randNum].genres;
        let genres = [];
        genreIDArray.forEach(genreID => {
            genres.push(genreData.data.genres[genreID].name);
        });
        ...
        solution.id = gameData.games[randNum].id;
        solution.game = gameData.games[randNum].game_title;
        solution.console = platformData.data.platforms[`${platformID}`].name;
        solution.franchise = findFranchise(gameData.games[randNum].game_title);
        solution.genres = genres;
        solution.releaseDate = gameData.games[randNum].release_date;
